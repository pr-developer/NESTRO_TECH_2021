...
function update() {
    ...
    function highlightConnected(g) {
        ...
        link.filter(function (d) {
            ...
            .style("opacity", OPACITY.LINK_DEFAULT);
        highlight_recursively(g);


    }

    function highlight_recursively(g) {
        g.targetLinks.forEach((r, i) => {
            link.filter(function (d) {
                return d.target === r.source;
            })
                .style("marker-end", function () {
                    return 'url(#arrowHeadOutlow)';
                })
                .style("stroke", INFLOW_COLOR)
                .style("opacity", OPACITY.LINK_DEFAULT);
            highlight_recursively(r.source);
        });
    }

    function fadeUnconnected(g) {
        link.filter(function (d) {
            return d.source !== g && d.target !== g && fadeUnconnected_recursively(g, d);
        })
            .style("marker-end", function () {
                return 'url(#arrowHead)';
            })
        ...
        node.filter(function (d) {
            var NodesArray = [];
            node_connectedArray(NodesArray, g);
            var pos = NodesArray.indexOf(d.name);
    ...
    function node_connectedArray(NodesArray, g) {
        g.targetLinks.forEach((r, i) => {
            NodesArray.push(r.source.name);
            node_connectedArray(NodesArray, r.source);
        });
    }

    function fadeUnconnected_recursively(g, d) {
        g.targetLinks.forEach((r, i) => {
            return d.source !== r.source && r.source !== g && fadeUnconnected_recursively(r.source, d);
        })
...
function getData() {
    ...
    client.get(URL_Host+'/nodes/?format=json', function (response) {
        ...
        for (var newExampleNode of jsnArray) {
            ...
            exampleNodes.push({
                "type": layer,
                "id": newExampleNode.id,
                "parent": null,
                "name": newExampleNode.label,
                "costdown": newExampleNode.costdown,
                "access": newExampleNode.access,
                "stead": stead,
                "xPOS": xPOS
            });
        }
        ...
        client1.get(URL_Host+'/edges/?format=json', function (response) {
            ...
            for (var newExampleNode of jsnArray) {
                exampleLinks.push({
                    "source": newExampleNode.source,
                    "target": newExampleNode.target,
                    "value": newExampleNode.weight
                });
            }
            build();
        });
